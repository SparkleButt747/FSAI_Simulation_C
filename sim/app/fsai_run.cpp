...
int main(int argc, char* argv[]) {
  ...
  std::function<void()> shutdown_imgui = []() {};
  if (renderer.window_enabled()) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ImGuiIO& io = ImGui::GetIO();
    (void)io;
    ImGui::StyleColorsDark();
    if (!ImGui_ImplSDL2_InitForSDLRenderer(renderer.window(),
                                           renderer.renderer())) {
  ...
  if (detection_preview_enabled) {
    if (!vision_node) {
    ...
    } else if (renderer.renderer() == nullptr) {
      fsai::sim::log::Logf(fsai::sim::log::Level::kWarning,
      ...
      std::string preview_error;
      if (!detection_preview.start(renderer.renderer(), vision_node, preview_error)) {
        if (preview_error.empty()) {
  ...
  while (running) {
    ...
    const auto wheel_speed_to_rpm = [wheel_radius](double speed_mps) {
      return static_cast<float>(speed_mps / (2.0 * std::numbers::pi * wheel_radius) * 60.0);
    };

    const double front_brake_torque = velox_telemetry.front_axle.brake_torque +
                                      velox_telemetry.front_axle.regen_torque;
    const double rear_brake_torque = velox_telemetry.rear_axle.brake_torque +
    ...
    const double front_brake_force = front_brake_torque / wheel_radius;
    const double rear_brake_force = rear_brake_torque / wheel_radius;
    const double front_net_force = front_drive_force - front_brake_force;
    const double rear_net_force = rear_drive_force - rear_brake_force;
    const double front_axle_torque_nm = velox_telemetry.front_axle.drive_torque - front_brake_torque;
    const double rear_axle_torque_nm = velox_telemetry.rear_axle.drive_torque - rear_brake_torque;

    const double total_brake_torque = front_brake_torque + rear_brake_torque;
    const double front_brake_pct = total_brake_torque > 0.0
                                       ? (front_brake_torque / total_brake_torque) * 100.0
                                       : 0.0;
    const double rear_brake_pct = total_brake_torque > 0.0
                                      ? (rear_brake_torque / total_brake_torque) * 100.0
                                      : 0.0;
    ...
    runtime_telemetry.pose.position_x_m = static_cast<float>(velox_telemetry.pose.x);
    runtime_telemetry.pose.position_y_m = static_cast<float>(velox_telemetry.pose.y);
    runtime_telemetry.pose.position_z_m = 0.0f;
    runtime_telemetry.pose.yaw_deg = static_cast<float>(velox_telemetry.pose.yaw *
                                                        fsai::sim::svcu::dbc::kRadToDeg);
    runtime_telemetry.wheels.rpm = {
    ...
    runtime_telemetry.acceleration.vertical_mps2 = 0.0f;
    ...
    std::array<float, 4> wheel_rpm_sample = runtime_telemetry.wheels.rpm;
    ...
    imu_sample.accel_longitudinal_mps2 = static_cast<float>(
        velox_telemetry.acceleration.longitudinal +
        sample_noise(sensor_cfg.imu.accel_longitudinal_std));
    ...
    telemetry.wheel_speed_rpm[0] = runtime_telemetry.wheels.rpm[0];
    telemetry.wheel_speed_rpm[1] = runtime_telemetry.wheels.rpm[1];
    telemetry.wheel_speed_rpm[2] = runtime_telemetry.wheels.rpm[2];
    telemetry.wheel_speed_rpm[3] = runtime_telemetry.wheels.rpm[3];
    ...
    telemetry.gps_lat_deg = static_cast<float>(
        metersToLatitude(velox_telemetry.pose.y));
    telemetry.gps_lon_deg = static_cast<float>(
        metersToLongitude(velox_telemetry.pose.x));
    telemetry.gps_speed_mps = static_cast<float>(vehicle_speed_mps);
