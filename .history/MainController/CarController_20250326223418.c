...
void CarController_Update(CarController* controller, double dt) {
    ...
    if (controller->useRacingAlgorithm) {
        ...
        // Use keyboard input instead of racing algorithm.
        int key = KeyboardInputHandler_GetInput();
        controller->throttleInput = 0.0f;
        controller->steeringAngle = 0.0f;
        if (key != -1) {
            if (key == 'w' || key == 'W') {
                controller->throttleInput = 1.0f;  // Accelerate
            } else if (key == 's' || key == 'S') {
                controller->throttleInput = -1.0f; // Decelerate
            }
            if (key == 'a' || key == 'A') {
                controller->steeringAngle = -1.0f; // Full left
            } else if (key == 'd' || key == 'D') {
                controller->steeringAngle = 1.0f;  // Full right
            }
        }
    }
