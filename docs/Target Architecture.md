...
- **Boundary Fit:** resample sides at 0.5–1.0 m, enforce monotone progress in arc length.
...
float u_throttle = RacingAlgorithm_GetThrottleInput(cps, N, s.vx_forward, &pose, &cfg, dt);
float u_steer    = RacingAlgorithm_GetSteeringInput  (cps, N, s.vx_forward, &pose, &cfg, dt);
