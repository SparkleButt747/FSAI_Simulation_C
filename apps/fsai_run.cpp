...
#include <io/can/can_link.hpp>
#include <control/ai2vcu_adapter.hpp>
#include <io/can/can_iface.hpp>
#include <apps/runtime_telemetry.hpp>
...
const char* CanModeToString(fsai::io::can::CanIface::Mode mode) {
  switch (mode) {
    case fsai::io::can::CanIface::Mode::kSimulation:
      return "Simulation";
    case fsai::io::can::CanIface::Mode::kFsAiApi:
      return "FS AI API";
  }
  return "Unknown";
}
...
int main(int argc, char* argv[]) {
  ...
  fsai::io::can::CanIface::Config can_cfg{};
  can_cfg.endpoint = can_iface;
  can_cfg.enable_loopback = true;
  can_cfg.mode = (mode == "car") ? fsai::io::can::CanIface::Mode::kFsAiApi
                                   : fsai::io::can::CanIface::Mode::kSimulation;
  if (can_cfg.mode == fsai::io::can::CanIface::Mode::kFsAiApi) {
    can_cfg.enable_loopback = false;
  }
  can_cfg.wheel_radius_m = vehicle_param.tire.radius;

  fsai::io::can::CanIface can_interface;
  if (!can_interface.Initialize(can_cfg)) {
    if (mode == "sim" && !can_iface_overridden) {
      ...
      can_cfg.mode = fsai::io::can::CanIface::Mode::kSimulation;
      if (!can_interface.Initialize(can_cfg)) {
  ...
  fsai::io::can::Ai2VcuCommandSet last_ai2vcu_commands{};
